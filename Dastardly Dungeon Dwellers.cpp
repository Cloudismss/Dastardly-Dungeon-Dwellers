...
// BEGIN TESTING PROTOTYPES & FUNCTIONS

void exampleFunctionCall(Progression &player);
Enemy *generateEnemyType(const Progression &p);

void exampleFunctionCall(Progression &player)
{
  Enemy *enemy = generateEnemyType(player);
  (*enemy).setEnemyAttributes();
  (*enemy).attackEnemy(10, "melee");
  player.attackPlayer(enemy->attackPlayer());
  enemy->debugPrint();
  enemy->announceEnemy();
  delete enemy;
}
...
// END TESTING PROTOTYPES & FUNCTIONS
