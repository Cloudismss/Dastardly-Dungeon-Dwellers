...
namespace art
{
  ...
  {  
    int menu(Player *player, Enemy *enemy)
    {
      ...
      do
      {
        displayHealth(player->getCharacter()->getClassName(), player->getCharacter()->getHealth(), enemy->getNickname(), enemy->getHealth());
        std::cout << "." << std::setfill('-') << std::setw(boxWidth) << ".\n"
                  << "|" << std::setfill(' ') << std::setw(boxWidth) << "|\n"
                  << "|" << std::setw(12) << " " << "1: " << choiceName[0] << std::setw(borderSpacing - choiceName[0].length()) << " " << "4: Heal" << std::setw(18) << " " << " |\n"
                  << "|" << std::setw(12) << " " << "2: " << choiceName[1] << std::setw(borderSpacing - choiceName[1].length()) << " " << "5: Run Away" << std::setw(14) << " " << " |\n"
                  << "|" << std::setw(12) << " " << "3: " << choiceName[2] << std::setw(borderSpacing - choiceName[2].length()) << " " << "6: Switch Character" << std::setw(18 - 12) << " " << " |\n"
                  << "|" << std::setfill(' ') << std::setw(boxWidth) << "|\n"
    ...
    void displayHealth(const string &className, int playerHealth, const string &enemyName, int enemyHealth)
    {
      ...
      else
      {
        ...
        std::cout << "." << std::setfill('-') << std::setw(leftGap + playerHealthDisplay.length()) << "." << std::setfill(' ') << std::setw(gapSize) << " " << "." << std::setfill('-') << std::setw(rightGap + enemyHealthDisplay.length()) << ".\n"
                  << "|" << std::setfill(' ') << std::setw(2) << " " << playerHealthDisplay << std::setw(2) << " " << "|" << std::setw(gapSize) << " " << "|" << std::setw(2) << " " << enemyHealthDisplay << std::setw(4) << "|\n"
                  << "'" << std::setfill('-') << std::setw(leftGap + playerHealthDisplay.length()) << "'" << std::setfill(' ') << std::setw(gapSize) << " " << "'" << std::setfill('-') << std::setw(rightGap + enemyHealthDisplay.length()) << "'\n"
                  << std::setfill(' ');
      }
    }
  }
