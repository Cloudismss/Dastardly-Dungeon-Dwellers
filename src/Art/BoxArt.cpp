...
void displayMeInABox(const std::string &message)
{
  ...
  std::cout << std::setw(indent) << "." << std::setfill('-') << std::setw(boxWidth) << ".\n"
            << std::setfill(' ') << std::setw(indent) << "|" << std::setfill(' ') << std::setw(boxWidth) << "|\n"
            << std::setw(indent) << "|" << std::setw(offset + borderSpacing - message.length() / 2) << " " << message
            << std::setw(borderSpacing - message.length() / 2) << " " << " |\n"
...
void displayMeInABox(const std::string &message1, const std::string &message2)
{
  ...
  std::cout << std::setw(indent) << "." << std::setfill('-') << std::setw(boxWidth) << ".\n"
            << std::setfill(' ') << std::setw(indent) << "|" << std::setfill(' ') << std::setw(boxWidth) << "|\n"
            << std::setw(indent) << "|" << std::setw(offset1 + borderSpacing - message1.length() / 2) << " " << message1
            << std::setw(borderSpacing - message1.length() / 2) << " " << " |\n"
            << std::setw(indent) << "|" << std::setw(offset2 + borderSpacing - message2.length() / 2) << " " << message2
            << std::setw(borderSpacing - message2.length() / 2) << " " << " |\n"
            << std::setw(indent) << "|" << std::setfill(' ') << std::setw(boxWidth) << "|\n"
            << std::setw(indent) << "'" << std::setfill('-') << std::setw(boxWidth) << "'\n"
            << std::setfill(' ') << "\n";
}
